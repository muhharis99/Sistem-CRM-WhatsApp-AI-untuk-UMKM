...
class BroadcastQueueService
{
    ...
    public function enqueueCampaign(int $tenantId,int $campaignId): array
    {
        ...
        $segment=json_decode((string)($campaign['segment_json']??'{}'),true)?:[];$query=$this->db->table('customers c')->select('c.id,c.phone,c.name,c.email,c.company,c.address,c.source,c.status,c.owner_id')->where('c.tenant_id',$tenantId)->where('c.phone !=','')->where("NOT EXISTS (SELECT 1 FROM opt_outs oo WHERE oo.tenant_id=c.tenant_id AND oo.customer_id=c.id AND oo.channel='WHATSAPP')",null,false);$this->applySegment($query,$segment,$tenantId);$customers=$query->get()->getResultArray();
