...
async function crm(path, data) {
  const response = await axios.post(`${CRM_BASE_URL}${path}`, data, { timeout: 20000, headers: { 'Content-Type': 'application/json', 'X-Queue-Internal-Secret': QUEUE_INTERNAL_SECRET }, validateStatus: () => true });
...
async function sendText(deviceId, remoteJid, text, tenantId) {
  const response = await axios.post(`${GATEWAY_URL}/api/whatsapp/messages/send`, { tenant_id: tenantId, device_id: deviceId, remote_jid: remoteJid, text }, { timeout: 20000, headers: { 'Content-Type': 'application/json', 'X-Gateway-Secret': GATEWAY_SECRET }, validateStatus: () => true });
...
const worker = new Worker('broadcast', async (job) => {
  ...
  try {
    ...
    await crm(`/api/internal/campaign-recipients/${recipient_id}/complete`, { tenant_id, status: 'SENT', message_id: result?.key?.id || null, attempts: recipient.attempts });
    return { success: true, recipient_id, message_id: result?.key?.id || null };
  } catch (error) {
    const retryable = job.attemptsMade + 1 < MAX_ATTEMPTS;
    await crm(`/api/internal/campaign-recipients/${recipient_id}/complete`, { tenant_id, status: retryable ? 'PENDING' : 'FAILED', error_message: error.message, attempts: recipient.attempts }).catch(() => null);
